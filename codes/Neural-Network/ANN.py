# -*- coding: utf-8 -*-
"""
Created on Fri Mar 18 00:43:06 2016
...
def cal_D(bias,alpha,ins,a2,a3,delta2,delta3,delta4,m,t1,t2,t3):
    br = np.repeat(bias,len(ins))
    x=np.concatenate((np.reshape(br,(len(ins),1)),ins),axis=1)
    t1 = t1 - (1.0/m)*alpha*(np.dot(delta2,x))      
    t2 = t2 - (1.0/m)*alpha*(np.dot(delta3,a2.T))   
    t3 = t3 - (1.0/m)*alpha*(np.dot(delta4.T,a3.T)) 
    return t1,t2,t3
...
for it in range(no_of_iter):
    ...
    theta1,theta2,theta3 = cal_D(bias,alpha,tr_data,a2,a3,d2,d3,d4,m,theta1,theta2,theta3)
    error_train[it] = (1.0/m)*np.sum(np.square(est_out-tr_label))
