# -*- coding: utf-8 -*-
"""
Created on Tue Mar 29 12:08:10 2016
...
theta = 2*np.random.random_sample((785,1))-1 #(np.zeros((785,1))) 
...
def GD(x,y,t,lemda):
    ...
    while loop <= len(er)-1:
        ...
        for j in range(785):
            ...
            t[j,0] = t[j,0] - alpha*(h_y*(1.0/m) - (lemda/m)*t[j,0]) 
        er[loop] = cost(x[:,1::],y,t,lemda)
...
def cost(x,y,theta,lemda):
    m = len(x)
    x = np.concatenate((np.ones((m,1)),x,),axis = 1)
    h = np.array(hyp(x,theta)).T
    y = y.reshape((m,1))
    c = 0.0
    c = (-1.0/m)*(np.sum(y*np.log(h) + (1-y)*np.log(1-h)) + lemda*np.sum(np.square(theta)))
    return np.nan_to_num(c)

lemda = 0.01
print "Applying Gradient descent... "
theta_est = GD(train_data01,train_label01,theta,lemda)
print "Finding error using cost function..."
error_train = cost(train_data01,train_label01,theta_est,lemda)
